...
namespace Grocery.App.ViewModels
{
    ...
    public partial class GroceryListItemsViewModel : BaseViewModel
    {
        ...
        private void GetAvailableProducts()
        {
            ...
            {
                bool isOnGroceryList = MyGroceryListItems.Any(item => item.ProductId == product.Id);

                // Als het product niet op de boodschappenlijst staat en de voorraad groter is dan 0
                if (!isOnGroceryList && product.Stock > 0)
                {
                    // Voeg het product toe aan de AvailableProducts lijst
        ...
        public void AddProduct(Product product)
        {
            ...
            // Werk de lijst AvailableProducts bij, want dit product is niet meer beschikbaar als de voorraad 0 is
            if (product.Stock == 0)
            {
                AvailableProducts.Remove(product);
            }

            // Voeg het nieuwe item toe aan de boodschappenlijst
            MyGroceryListItems.Add(newGroceryListItem);

            // Call OnGroceryListChanged om de lijst opnieuw te laden
